...
void translate(int lineNum){
	if(strcmp(IRCodeArray[lineNum].operator,"scanint")==0){
...
int main(int argc, char const *argv[])
{
	...
	while ((read = getline(&line, &len, fp)) != -1) {
	    ...
	    token = strtok(NULL, ", ");
   		if(strcmp(token,"function")==0)continue;
   		if(strcmp(token,"scanint")==0)continue;
   		if(strcmp(token,"call")==0)continue;
   		if(strcmp(token,"goto")==0)continue;
   		if(strcmp(token,"label")==0)continue;
   		if(strcmp(token,"readarr")==0 || strcmp(token,"writearr")==0)
	    	token = strtok(NULL, ", ");
   		if(strcmp(token,"ifgoto")==0){
	    	token = strtok(NULL, ", ");
	    	token = strtok(NULL, ", ");
   			if(!isValidNumber(token))pushVarNode(&varHead,token);
   	    	token = strtok(NULL, ", ");
	...
	while ((read = getline(&line, &len, fp)) != -1) {
	    ...
	    if(strcmp(token,"function")==0 || strcmp(token,"call")==0 || strcmp(token,"label")==0){
   		...
   		}else if(strcmp(token, "scanint") == 0){
	...
	for (int i = 0; i < LINES; ++i)
	{
		if(strcmp(IRCodeArray[i].operator, "ifgoto")==0){
			...
			isLeader[i] = true;
				...
				strcmp(IRCodeArray[i].operator, "scanint")==0){
			if(i==LINES-1)break;
			isLeader[i+1] = true;
		}
	...
	for(int i=0;i<leaderCount;i++){
		...
		for(int k=startIns;k<=endIns;k++){
			// printf("came for %d\n", k+1);
			// if(k==endIns)printf("last in block ---------------------\n");
			// if(k==endIns && strcmp(IRCodeArray[k].operator,"ifgoto")!=0 &&
			// 				strcmp(IRCodeArray[k].operator,"print")!=0 ){
			// 	// printf("entered for ins %d\n", k+1);
			// 	for (int j = 0; j < VARIABLES_COUNT; ++j)
			// 	{
			// 		printf("%s is in %s\n",AddrDescriptor[j].varName, AddrDescriptor[j].address);
			// 		if(strcmp(AddrDescriptor[j].address,"mem")!=0){
			// 			printf("sw $t%d, %s\n", getRegNum(AddrDescriptor[j].varName), AddrDescriptor[j].varName);
			// 			setAddDesc(AddrDescriptor[j].varName, "mem");
			// 		}
